...
"""
Can pass either a Surface object or a filename as the image argument
"""
def splice_image(image, frame_width, frame_height, margin_x=0, margin_y=0):
    if isinstance(image, basestring):
        image = peachy.graphics.get_image(image)
        if image is None:
            raise IOError("splice_image problem")
    ...
    while x + frame_width <= src_width and y + frame_height <= src_height:
        crop = peachy.graphics.Surface((frame_width, frame_height), flags=pygame.SRCALPHA)
